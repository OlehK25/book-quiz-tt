...
```mermaid
graph TB
    A[👤 User Input<br/>Email] -->|Введення даних| B[🔐 encrypt email]
    
    B --> C1[Generate IV<br/>random]
    B --> C2[Fetch session key]
    B --> C3[AES-GCM encryption]
    
    C1 --> D[💾 localStorage<br/>encrypted]
    C2 --> D
    C3 --> D
    
    D -->|Зашифровано| E["🔒 ZX4jK...9mQ==<br/>cipher текст"]
    ...
    subgraph "Session Key Lifecycle"
        SK1[🔑 Генерується при першому<br/>завантаженні квізу]
        SK2[💼 Зберігається у<br/>sessionStorage]
        SK3[🗑️ Знищується при<br/>закритті вкладки]
        SK1 --> SK2 --> SK3
    end
